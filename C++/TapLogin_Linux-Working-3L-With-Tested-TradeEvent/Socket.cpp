...
{
    //TCPSocket(remoteAdd,remotePort);
    connect(remoteAdd,remotePort);
}
...
// For Reusable Port -> First setsockopt after that Bind the Socket
...
void UDPSocket::setBroadcast()
{
 ...
 if (setsockopt(sockDesc, SOL_SOCKET, SO_REUSEADDR, (raw_type *) &broadcastPermission, sizeof(broadcastPermission))<0)
...
void UDPSocket::joinGroup(const string &multicastGroup,const string &source_iface) throw(SocketException) {
	struct ip_mreq multicastRequest;
	//string source_iface="127.0.0.1";

  multicastRequest.imr_multiaddr.s_addr = inet_addr(multicastGroup.c_str());
  multicastRequest.imr_interface.s_addr = (source_iface.empty() ? htonl(INADDR_ANY) : inet_addr(source_iface.c_str()));
	  //htonl(INADDR_ANY);
